...
// 默认模糊数据URL - 一个简单的灰色渐变base64
const DEFAULT_BLUR_DATA_URL = 'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIzMDAiIGhlaWdodD0iNTAwIiBmaWxsPSJub25lIiBvcGFjaXR5PSIwLjEiPjxjaXJjbGUgY3g9IjE1MCIgY3k9IjI1MCIgcj0iMTUwIiBmaWxsPSIjMDAwIi8+PC9zdmc+';
...
export default function VideoCard({
  ...
  // 实现IntersectionObserver监听，对即将进入视口的视频卡片预加载图片
  useEffect(() => {
    if (typeof IntersectionObserver === 'undefined') {
      // 对于不支持IntersectionObserver的浏览器，直接设置为可见
      setIsVisible(true);
      return;
    }
  ...
  return (
    <div
      ref={cardRef}
      className='group relative w-full rounded-lg bg-transparent cursor-pointer transition-all duration-300 ease-in-out hover:scale-[1.05] hover:z-[500] overflow-hidden'
      onClick={handleClick}
    >
      {/* 海报容器 */}
      <div className='relative aspect-[2/3] overflow-hidden rounded-lg'>
        ...
        <Image
          ...
          alt={actualTitle}
          fill
          className={`object-cover ${!isVisible ? 'opacity-0' : 'opacity-100'}`}
