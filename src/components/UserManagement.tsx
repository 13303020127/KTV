...
export default function UserManagement() {
  ...
  // 使用React Query获取用户列表
  const { data: usersData, isLoading, error, refetch } = apiService.getUsers();
  const updateUserSettings = apiService.updateAdminUserSettings();
  ...
  const handleForceFilter = (username: string) => {
    if (confirm(`确定要强制开启用户 ${username} 的成人内容过滤功能吗？`)) {
      handleUpdateUserSettings(username, 'force_filter');
    }
  };

  const handleAllowDisable = (username: string) => {
    if (confirm(`确定要允许用户 ${username} 自己管理过滤设置吗？`)) {
      handleUpdateUserSettings(username, 'allow_disable');
    }
  };
