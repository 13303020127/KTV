...
export class LocalStorage implements IStorage {
  ...
  async getPlayRecord(userName: string, key: string): Promise<PlayRecord | null> {
    ...
    try {
      ...
      const data = optimizedStorage.getItem(storageKey);
      return data; // 优化存储已经处理了解析
    } catch (error) {
  ...
  async setPlayRecord(userName: string, key: string, record: PlayRecord): Promise<void> {
    ...
    try {
      const storageKey = this.getStorageKey('playrecord', userName, key);
      optimizedStorage.setItem(storageKey, record);
    } catch (error) {
  ...
  async deletePlayRecord(userName: string, key: string): Promise<void> {
    ...
    try {
      const storageKey = this.getStorageKey('playrecord', userName, key);
      optimizedStorage.removeItem(storageKey);
    } catch (error) {
  ...
  async setFavorite(userName: string, key: string, favorite: Favorite): Promise<void> {
    ...
    try {
      const storageKey = this.getStorageKey('favorite', userName, key);
      optimizedStorage.setItem(storageKey, favorite);
    } catch (error) {
  ...
  async registerUser(userName: string, password: string): Promise<void> {
    ...
    try {
      ...
      optimizedStorage.setItem(storageKey, userData);
    } catch (error) {
  ...
  async verifyUser(userName: string, password: string): Promise<boolean> {
    ...
    try {
      const storageKey = this.getStorageKey('user', userName);
      const userData = optimizedStorage.getItem(storageKey);
  ...
  async checkUserExist(userName: string): Promise<boolean> {
    ...
    try {
      const storageKey = this.getStorageKey('user', userName);
      return optimizedStorage.getItem(storageKey) !== null;
    } catch (error) {
  ...
  async addSearchHistory(userName: string, keyword: string): Promise<void> {
    ...
    try {
      ...
      const storageKey = this.getStorageKey('searchhistory', userName);
      optimizedStorage.setItem(storageKey, limitedHistory);
    } catch (error) {
  ...
  async deleteSearchHistory(userName: string, keyword?: string): Promise<void> {
    ...
    try {
      ...
      if (!keyword) {
        ...
        optimizedStorage.setItem(storageKey, newHistory);
      }
    } catch (error) {
  ...
  async setSkipConfig(userName: string, key: string, config: EpisodeSkipConfig): Promise<void> {
    ...
    try {
      const storageKey = this.getStorageKey('skipconfig', userName, key);
      optimizedStorage.setItem(storageKey, config);
    } catch (error) {
  ...
  async setUserSettings(userName: string, settings: UserSettings): Promise<void> {
    ...
    try {
      const storageKey = this.getStorageKey('settings', userName);
      optimizedStorage.setItem(storageKey, settings);
    } catch (error) {
  ...
  async getAdminConfig(): Promise<AdminConfig | null> {
    ...
    try {
      const data = optimizedStorage.getItem('katelyatv_admin_config');
      return data;
    } catch (error) {
  ...
  async setAdminConfig(config: AdminConfig): Promise<void> {
    ...
    try {
      optimizedStorage.setItem('katelyatv_admin_config', config);
    } catch (error) {
  ...
  async deleteUser(userName: string): Promise<void> {
    ...
    try {
      ...
      optimizedStorage.removeItem(userKey);
      ...
      for (const prefix of prefixes) {
        ...
        keysToRemove.forEach(key => optimizedStorage.removeItem(key));
      }
    } catch (error) {
