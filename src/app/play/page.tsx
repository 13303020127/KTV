...
function PlayPageClient() {
  ...
  // 去广告开关（默认为true，在useEffect中从localStorage加载）
  const [blockAdEnabled, setBlockAdEnabled] = useState<boolean>(true);
  
  // 仅在客户端加载localStorage中的设置
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const v = localStorage.getItem('enable_blockad');
      if (v !== null) {
        setBlockAdEnabled(v === 'true');
      }
    }
  }, []);
  ...
  const preloadedEpisodesRef = useRef<Set<number>>(new Set()); // 用于跟踪已预加载的集数
  ...
  useEffect(() => {
    ...
    const initAll = async () => {
      ...
      // 安全检查：确保episodes存在且是数组
      if (detailData.episodes && Array.isArray(detailData.episodes) && currentEpisodeIndex >= detailData.episodes.length) {
  ...
  const handleSourceChange = async (
    ...
    try {
      ...
      if (!newDetail.episodes || !Array.isArray(newDetail.episodes) || targetIndex >= newDetail.episodes.length) {
        targetIndex = 0;
      }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const preloadNextEpisode = () => {
        ...
        // 如果已经预加载过，不再重复预加载
        if (preloadedEpisodesRef.current.has(idx + 1)) {
          return;
        }
        
        // 获取下一集URL
        const nextEpisodeUrl = d.episodes[idx + 1];
        if (nextEpisodeUrl) {
          // 创建预加载请求
          const preloadLink = document.createElement('link');
          preloadLink.rel = 'preload';
          preloadLink.as = 'fetch';
          preloadLink.href = nextEpisodeUrl;
          ...
          setTimeout(() => {
            ...
            // 标记为已预加载，避免重复预加载
            preloadedEpisodesRef.current.add(idx + 1);
          }, 30000); // 30秒后清理
          
          console.log('开始预加载下一集:', idx + 2);
        }
      };
